class JkciClassesController < ApplicationController
  ...
  def get_activities
    ...
    activities = PublicActivity::Activity.includes(:owner).where("(trackable_type like 'JkciClass' && trackable_id = ?) || (recipient_type like 'JkciClass' && recipient_id = ?)", jkci_class.id, jkci_class.id)
