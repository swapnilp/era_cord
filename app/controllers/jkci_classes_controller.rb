class JkciClassesController < ApplicationController
  ...
  def get_timetable
    ...
    if time_table
      render json: {success: true, time_table: time_table.as_json, subjects: jkci_class.subjects.as_json, slots: time_table.time_table_classes.as_json, subjects: jkci_class.subjects.as_json}
    else
      render json: {success: false}
    end
