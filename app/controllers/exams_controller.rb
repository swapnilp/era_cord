class ExamsController < ApplicationController
  ...
  def index
    if params[:jkci_class_id].present?
      ...
      exams = jkci_class.exams.includes([:subject, :exam_catlogs]).order("exam_date desc").page(params[:page]) #@organisation.exams.roots.order("id desc").page(params[:page])
    else
      exams = Exam.all.order("exam_date desc").page(params[:page]) #@organisation.exams.roots.order("id desc").page(params[:page])
    end
