class Exam < ActiveRecord::Base
  ...
  def publish_results
    ...
    if self.root?
      if self.jkci_class.enable_exam_sms && self.organisation.root.is_send_message
        if self.is_group
  ...
  def group_result_message_send
    ...
    self.grouped_exams_sms.each do |report|
      message = report[0]
      url = "https://www.txtguru.in/imobile/api.php?username=#{SMSUNAME}&password=#{SMSUPASSWORD}&source=eracod&dmobile=#{report[1]}&message=#{message}"
      student_id = report[2]
  ...
  def self.json(node)
    {id: node.id, name: node.name, marks: node.marks, subject: node.subject.try(:only_std_name), 
      exam_date: node.exam_date.strftime("%d %b %Y @ %I:%M %p"), exam_type: node.exam_type, 
      ...
      chapters_points: node.chapters_points.map(&:chapter_name), descendants_count: node.descendants.count
    }
  end
